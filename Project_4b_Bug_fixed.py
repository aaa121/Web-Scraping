'''
The Project 4_bs4_and_requests.py is modified for bugs as follows:
The bugs were noted based on the differences in the elements of the objects; Name=35; Address_modified=35;Phone=30;YP_Rating=29
(1). The coffee shop "The Downbeat cafe" has only YP rating not with trip advisor rating. This makes the element of the cafe
to has different class as "<a class=rating </a>" instead of "<a class=rating hasExtraRating  </a>". This adjusted in the for loop:
sub_data2 = rate1.find_all("a", {"class": "rating"})

(2). Fixing the first changed the lengths of the objects to Name=35; Address_modified=35;Phone=30;YP_Rating=33. The actual data of
interest is 30 results per page. The last five is teh side bar adverts. To fix this reduce the elements to 30 i.e. object[:-5]
'''
...
Name=Name[:30]
Address_modified=Address_modified[:30]
Phone_Number=Phone_Number[:]
YP_Rating=YP_Rating[:30]
